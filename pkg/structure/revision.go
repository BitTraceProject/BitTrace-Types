...
type (
	// Revision 同一个 Snapshot 期间，每一个过程结束时输出一次，Revision 将 Status 迁移，Event，Result 等打包输出
	Revision struct {
		Structure
...
// ReceiveEvent 是在 event 对应的 result 还未发生，event 此时到达了 revision
...
// CommitStatusTransfer 是在某一 event 对应的 result 已经发生了后调用的，将 event 与 result 关联起来，然后将 transfer 添加到 list
